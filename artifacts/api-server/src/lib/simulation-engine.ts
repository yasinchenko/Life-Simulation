...
// ─── Dialog mood matrix ────────────────────────────────────────────────────
// Эффект диалога зависит от тиров настроения инициатора и ответчика.
// Формат: [dInitMin, dInitMax, dRespMin, dRespMax]
const MOOD_TIER_HIGH = 65;
const MOOD_TIER_LOW  = 35;
...
const DIALOG_MATRIX: Record<MoodTier, Record<MoodTier, [number, number, number, number]>> = {
  //                          dInit_min  dInit_max  dResp_min  dResp_max
  high: {
    high: [ 3,  6,  3,  6],  // взаимная радость — оба счастливы
    med:  [ 1,  4,  2,  4],  // позитивный поднимает нейтрального
    low:  [-1,  2,  1,  5],  // позитивный «вкладывается», грустный получает
  },
  med: {
    high: [ 2,  4,  0,  1],  // нейтральный тянется к позитивному
    med:  [-1,  3, -1,  3],  // нейтральный диалог — может пойти в любую сторону
    low:  [-2,  1, -1,  2],  // нейтральный слегка теряет, грустный немного получает
  },
  low: {
    high: [-3,  0, -1,  1],  // грустный немного «заражает» позитивного
    med:  [-4, -1, -2,  1],  // грустный тянет нейтрального вниз
    low:  [-5, -2, -5, -2],  // взаимная негативность — оба становятся хуже
  },
};
// ──────────────────────────────────────────────────────────────────────────
    ...
    for (const agentId of agentIds) {
      ...
      if (criticalNeed === "sleep") {
        ...
        if (partnerId) {
          ...
          if (partner) {
            // ── Матрица диалогов: тир инициатора × тир ответчика ──
            const initTier = getMoodTier(agent.mood);
            const respTier = getMoodTier(partner.mood);
            const [diMin, diMax, drMin, drMax] = DIALOG_MATRIX[initTier][respTier];

            // Применяем socialInteractionStrength как масштабный коэффициент
            const s = socialInteractionStrength * 0.5; // нормализуем: стандарт=1x при strength=2
            const dMoodInit = rand(diMin, diMax) * s;
            const dMoodResp = rand(drMin, drMax) * s;
            ...
            // Отношения: положительный диалог укрепляет связь, отрицательный — ослабляет
            const relationDelta = (dMoodInit + dMoodResp) * 1.5;
            this.updateRelation(agentId, partnerId, relationDelta);
            this.updateRelation(partnerId, agentId, relationDelta * 0.5);
          }
        }
